...
#region Initialization                                                                              //Initialization
        public void Initialize(CharacterSystemManager manager)                                      //Initialize the CharacterSystemManager to get references to the other components
        {
            _attributeSystem = manager.AttributeSystem;                                                //reference to the attribute system for managing attribute points on level up
            _characterSheet = manager.Sheet;                                                           //reference to the character sheet for fetching character name and updating level and xp info on the sheet.                        
        }
...
#region Core LvlSystem
    ...
    public void LevelUp()                                                                        //Method for handling Level Ups 
    {
        ...
        OnCharacterLevelUpdate?.Invoke(_lvl);                                                       //notify CharSheet.cs about the info change
          if (_currentXP >= _xpToNextLVL)                                                                 //check if the xp overflow from the level up is enough to trigger another level up, if so call LevelUp() recursively until the overflow xp is less than the xp required for the next level.
        {
            LevelUp();
        }
...
#region Public Getters
    public int GetCharacterLevel()                                                            //Method to return the current level if needed.
    {
        if (lvlSystemDebugMode)
        {
            Debug.Log(_characterName + " is LVL: " + _lvl);                                                 
        }
        return _lvl;   
    }
    ...
    public int GetLifetimeTotalXP()                                                           //Method to return the lifetime total XP if needed.
    {
        if (lvlSystemDebugMode)
        {
            Debug.Log(_characterName + " Lifetime Total XP: " + _lifetimeXp);                               
        }
        return _lifetimeXp;   
    }
#endregion
...
#region UI Update
    void UpdateUI()                                                                                     //Method for updating Ui elements, consider moving to its own component.
        {
            if (nameText == null || lvlText == null || xpText == null || xpSlider == null)                     //if any UI reference is not found throw a warning and return.
            {
                if (lvlSystemDebugMode) Debug.LogWarning(                                                      
                    "LVLSystem.UpdateUI: UI references not assigned; skipping UpdateUI.");
                return;
            }
            ...
            if (lvlSystemDebugMode)
            {
                Debug.Log("Fill Value: " + fillValue + " | Start: " + _start + " | End: " + _end +
